...
from .models import User,Profilepic,Crops,Categories
...
def home(request):
    farmers = User.objects.filter(role='Farmer')[:6]
    for farmer in farmers:
        profilepic=Profilepic.objects.filter(user=farmer)
    dealers = User.objects.filter(role='Dealer')[:6]
    for dealer in dealers:
        delaerprofilepic=Profilepic.objects.filter(user=dealer)
    ...
    # Annotate the products with total_quantity and total_rate
    products_info = Product.objects.values('product_name').distinct().annotate(total_quantity=Sum('quantity'), total_rate=Sum('rate'))

    # Initialize the dictionary to hold product details and related crops
    product_crops_dict = {}
    ...
    for product_info in products_info:
        ...
        # Get related crops based on product name
        crops = Crops.objects.filter(nameineng=product_name)
        # Add to the dictionary
        product_crops_dict[product] = {
            'product_info': product_info,
            'crops': list(crops)
        }


    return render(request, 'home_accounts.html',{'farmers':farmers,'profilepic': profilepic,'dealers':dealers,'delaerprofilepic':delaerprofilepic,'products_info':products_info,'latest_messages': latest_messages,'product_crops_dict': product_crops_dict})
