...
from accounts.models import User,Subscription,Profilepic,Categories,Crops
...
def farmerprofile(request):
        try:
                ...
                return render(request, 'farmerprofile_farmer.html', {'profile': profile,'profilepic':profilepic,'subscription':subscription})
...
def crops(request,id):
        ...
        return render(request,'subcategories_farmer.html',{'user_name':user_name,'crops':crops,'farmer':farmer}) 
...
def farmerbit(request):
    ...
    if user_id:
        try:
            user = User.objects.get(id=user_id)
            bit_data = Bit.objects.filter(farmer_id=user,status='false').values('id', 'product', 'bitter', 'product_type', 'quantity', 'rate', 'quality', 'bit_value', 'status')
...
def bithistory(request):
        ...
        return render(request,'bithistory_farmer.html',{'bits_by_year': bits_by_year} )
...
#                                 if (subscription is not None):
#                                         return render(request, 'farmerprofile_farmer.html', {'profile': profile,'subscription':subscription,'date':date,'expiry_date':expiry_date})
...
def subscription(request):
        try:
                ...
                subscription=Subscription.objects.filter(user=profile)
                profilepic=Profilepic.objects.get(user=profile) 
                return render(request, 'farmerprofile_farmer.html', {'profilepic':profilepic,'profile':profile,'subscription':subscription})                       
...
def freeplan(request):          
        ...
        if not existing_subscription:
                ...
                try:
                        ...
                        subscription=Subscription.objects.filter(user=profile)
                        return render(request, 'farmerprofile_farmer.html', {'profile': profile,'profilepic':profilepic,'subscription':subscription})
        return redirect(request,'subcription_farmer.html')       
...
def extendsubscription(request):
       ...
       return render(request,'extenedsubscription_farmer.html',{'user_subscription':user_subscription,'user_name':user_name})
...
def standardplan(request,amount):  
        ...
        # if subscribe:
        #         new_plan = request.POST.get('planname')
        #         new_duration = 30  # Duration in days
        #         amount = 999
...
def premiumplan(request,amount):        
        ...
        #         user = request.user
        #         new_plan = request.POST.get('planname')
        #         new_duration = int(request.POST.get('duration'))  # Duration in days
        #         amount = request.POST.get('amount')
...
def demo(request):
        ...
        client = razorpay.Client(auth=('rzp_test_bX75Gd98qBwkpY', 'dqDmwLhAXqBPTz1okdtBUHMJ'))
        payment = client.order.create({'amount':(amount)*100,'currency':"INR",'payment_capture':'1'})
        print('heyyyiiiii',payment)
        order_id=payment['id']
        order_status=payment['status']
        return render(request,'demo.html',{'payment':payment})


def success(request,payment_id,plan,order_id,signature):
    user_id=request.session.get('user_id')
    user=User.objects.get(id=user_id)
...
def farmeragreement(request):
        ...
        if user_id:
                ...
                if user_subscription:
                        ...
                        if current_date > expiry_date:
                                Product.objects.filter(user=user).delete()
                                return render(request, 'subscription_farmer.html',{'user_name':user_name,'user_subscription':user_subscription})
